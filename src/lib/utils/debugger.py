...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 3 or dataset == 'carla':
      self.names = carla_class_name
    elif num_classes == 2 or dataset == 'dair-v2x':
      self.names = dairv2x_class_name
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    if img_id not in self.bbox_counts:
        self.bbox_counts[img_id] = {}
    if cat not in self.bbox_counts[img_id]:
        self.bbox_counts[img_id][cat] = 0
    self.bbox_counts[img_id][cat] += 1

  def add_bev_points(self, x_coords_dict, y_coords_dict, img_id='bev'):
    bev_img = np.ones((512, 1024, 3), dtype=np.uint8) * 255

    center = (1024 // 2, 512)
    half_fov = np.deg2rad(90 / 2)
    line_length = 768

    left_endpoint = (int(center[0] - line_length * np.sin(half_fov)), int(center[1] - line_length * np.cos(half_fov)))
    right_endpoint = (int(center[0] + line_length * np.sin(half_fov)), int(center[1] - line_length * np.cos(half_fov)))

    cv2.line(bev_img, center, left_endpoint, (0, 0, 0), 2)
    cv2.line(bev_img, center, right_endpoint, (0, 0, 0), 2)

    bbox_counts = self.bbox_counts.get('ctdet', {})

    for bbox_class_id in bbox_counts.keys():
        num_bboxes = bbox_counts[bbox_class_id]

        x_coords_class_id = category_mapping.get(bbox_class_id, None)

        if x_coords_class_id is not None and x_coords_class_id in x_coords_dict and x_coords_class_id in y_coords_dict:
            x_coords = x_coords_dict[x_coords_class_id][:num_bboxes]
            y_coords = y_coords_dict[x_coords_class_id][:num_bboxes]
            if isinstance(y_coords, np.float32):
              y_coords = [y_coords * 512 / 100]
            else:
              y_coords = y_coords * 512 / 100
            if isinstance(x_coords, np.float32):
              x_coords = [x_coords * 1024 / 200]
            else:
              x_coords = x_coords * 1024 / 200
            ...
            if bbox_class_id == 1:
              color = (255, 0, 0)
              box_size = 10
            elif bbox_class_id == 0:
              color = (0, 0, 255)
              box_size = 5
            for x, y in zip(x_coords, y_coords):
                point = (int(526 - x), int(512 - y))
                cv2.rectangle(bev_img, (int(point[0] - box_size / 2), int(point[1] - box_size / 2)), (int(point[0] + box_size / 2), int(point[1] + box_size / 2)), color, 2)

    self.imgs[img_id] = bev_img
...
carla_class_name = ['pedestrian', 'car', 'truck']

dairv2x_class_name = ['car', 'pedestrian']
