...
def _topk(scores, K=40):
    ...
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K) # scores: (batch, cat, (height x width))
                                                                        # topk_inds: (batch, cat, K)
    topk_inds = topk_inds % (height * width)                            # topk_scores: (batch, cat, K)
    topk_ys   = (topk_inds / width).int().float()
    topk_xs   = (topk_inds % width).int().float()
      
    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K) # topk_score: (batch, K)  topk_ind: (batch, K)
...
def ctdet_decode(heat, wh, dis, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    dis_dict = {j: [] for j in range(cat + 1)}
    ...
    for j in range(cat + 1):
        for i in range(K):
            dis_value = dis[0][0][ct_y[0][i], ct_x[0][i]].cpu().numpy()
            dis_dict[j].append(dis_value)

    # Convert lists to numpy arrays
    for j in dis_dict:
        dis_dict[j] = np.array(dis_dict[j], dtype=np.float32)
    if reg is not None:
      reg = _transpose_and_gather_feat(reg, inds)
      reg = reg.view(batch, K, 2)
      xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
